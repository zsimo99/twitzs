...
const FormControle = ({id,value,setFormData,name,type}) => {
  ...
  return (
    ...
    <div className="relative w-full">
        <label className={`text-white z-[-1] absolute left-4 transition-all ${(focused || !vide) ? ("top-0 translate-y-[-50%] text-[12px] bg-slate-800 px-1 z-[10]") : ("top-[50%] translate-y-[-50%] text-sm")}`} htmlFor="email">{name}</label>
